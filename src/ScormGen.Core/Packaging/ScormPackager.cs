...
public class ScormPackager
{
    ...
    private static byte[] BuildPackageZip(ScormPackage pkg)
    {
        ...
        var manifest = HtmlTemplates.GetManifest(pkg.FileName, pkg.Title, pkg.ContentType, pkg.PassingScore);

        using var ms = new MemoryStream();
        using (var zip = new ZipArchive(ms, ZipArchiveMode.Create, leaveOpen: true))
        {
            AddEntry(zip, "index.html", html);
            AddEntry(zip, "scorm_api.js", HtmlTemplates.ScormApiJs);
    ...
    // -------------------------------------------------------------------------
    // HTML generation — mirrors Python SCORMGenerator methods
    // -------------------------------------------------------------------------

    private static string GenerateInformational(ScormPackage pkg)
    {
    ...
    private static string GenerateScenario(ScormPackage pkg)
    {
...
"""
    ...
    private static string GenerateUngradedQuiz(ScormPackage pkg)
    {
        var questions = pkg.Content.OfType<MultipleChoice>().ToList();
        var questionsHtml = BuildQuizQuestionsHtml(questions, graded: false);
        return HtmlTemplates.GetUngradedQuizHtml(pkg.Title, questionsHtml, questions.Count);
    }

    private static string GenerateGradedQuiz(ScormPackage pkg)
    {
        var questions = pkg.Content.OfType<MultipleChoice>().ToList();
        ...
        // Build JSON dicts keyed by 1-based question index
